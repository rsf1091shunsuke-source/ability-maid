  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: 24, padding: '32px 48px', textAlign: 'center' as const,
      }}>
...
export default function GamePage() {
  ...
  const [selectingHandover, setSelectingHandover] = useState(false);
  ...
  useEffect(() => {
    if (!game || !myId) return;
    if (game.exposeTarget === myId) showMsg('💀 暴露！あなたの手札が相手に5秒間公開されています！');
  ...
  useEffect(() => {
    ...
    if (!challenge || challenge.resolved || challenge.usedBy === myId || challenge.intercepted !== null) return;
  ...
  useEffect(() => {
    ...
    if (challenge.intercepted !== null) {
      if (challenge.intercepted === 'reflect') showMsg('🛡 反射された！');
      else showMsg('🚫 無効化された！');
      updateDoc(doc(db, 'abilityMaidGames', id), { abilityChallenge: null });
      return;
    }
    const delay = challenge.expiresAt - Date.now();
    if (delay <= 0) { executeAbilityAfterChallenge(challenge.ability); return; }
    const timer = setTimeout(() => executeAbilityAfterChallenge(challenge.ability), delay);
  ...
  const opponentHasAbility = (ability: AbilityType) => (opponent?.availableAbilities || []).includes(ability);
  ...
  const handlePairFormed = async (paired: AbilityType, myHand: CardType[], oppHand: CardType[], extraUpdates: Record<string, unknown> = {}) => {
    if (paired === 'expose') {
      showMsg('💀 暴露発動！あなたの手札が5秒間公開されます！');
      await updateDoc(doc(db, 'abilityMaidGames', id), {
        [`players.${myId}.hand`]: myHand,
        [`players.${opponentId}.hand`]: oppHand,
        exposeTarget: myId, ...extraUpdates, ...checkWin(myHand, oppHand),
      });
      setTimeout(async () => await updateDoc(doc(db, 'abilityMaidGames', id), { exposeTarget: null }), 5000);
    } else {
      showMsg(`⚡ ${ABILITY_INFO[paired]?.icon} ${ABILITY_INFO[paired]?.name} が使えるようになった！`);
      await updateDoc(doc(db, 'abilityMaidGames', id), {
        [`players.${myId}.hand`]: myHand,
        [`players.${myId}.availableAbilities`]: [...(myPlayer?.availableAbilities || []), paired],
        [`players.${opponentId}.hand`]: oppHand,
        ...extraUpdates, ...checkWin(myHand, oppHand),
      });
    }
  };

  const drawFromOpponent = async (index: number) => {
    if (!isMyTurn || turnPhase !== 'draw_opponent' || processing || selectingDecoy || selectingHandover) return;
    ...
    const info = actualCard !== 'joker' ? ABILITY_INFO[actualCard as AbilityType] : null;
    showMsg(`${actualCard === 'joker' ? '🃏 ジョーカー' : `${info?.icon ?? ''} ${info?.name ?? actualCard}`} を引いた！`);
  ...
  const useAbility = async (ability: AbilityType) => {
    if (!checkCondition(ability, myPlayer.hand, opponent?.hand || [])) {
      showMsg(`⚠️ 条件未達成：${ABILITY_INFO[ability].condition}`); return;
    }
    ...
    await updateDoc(doc(db, 'abilityMaidGames', id), { [`players.${myId}.availableAbilities`]: newAvail });
    if (opponentCanIntercept) {
      await updateDoc(doc(db, 'abilityMaidGames', id), {
        abilityChallenge: { ability, usedBy: myId, expiresAt: Date.now() + 4000, intercepted: null, resolved: false }
      });
  ...
  const executeAbility = async (ability: AbilityType) => {
    switch (ability) {
      case 'spy':
        if (game.blackoutActive) { showMsg('🌑 情報封鎖で無効化！'); break; }
      ...
      case 'swap':
        ...
        break;
      case 'handover':
        if (myPlayer.hand.length > 0) { setSelectingHandover(true); showMsg('🤝 相手に渡すカードを選んでください'); }
        break;
      case 'decoy':
  ...
  const selectHandoverCard = async (index: number) => {
    if (!selectingHandover) return;
  ...
  if (game.status === 'waiting') {
    return (
      <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, padding: 24, background: '#0a0a1a' }}>
        ...
        <p style={{ fontSize: 20, fontWeight: 700, color: '#fff' }}>対戦相手を待っています...</p>
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', padding: 16, gap: 12, maxWidth: 480, margin: '0 auto', paddingBottom: 32, background: '#0a0a1a' }}>
        ...
        <div style={{ position: 'fixed', bottom: 80, left: '50%', transform: 'translateX(-50%)', zIndex: 150, width: '90%', maxWidth: 420, background: '#1a0a2e', border: '2px solid #9b59b6', borderRadius: 16, padding: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {availableAbilities.includes('reflect') && (
              <button onClick={() => interceptAbility('reflect')} style={{ flex: 1, padding: '10px', borderRadius: 10, border: '1px solid #4A90E2', background: 'rgba(74,144,226,0.2)', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>🛡 反射する</button>
            )}
            {availableAbilities.includes('nullify') && (
              <button onClick={() => interceptAbility('nullify')} style={{ flex: 1, padding: '10px', borderRadius: 10, border: '1px solid #E53935', background: 'rgba(229,57,53,0.2)', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>🚫 無効化する</button>
            )}
            <button onClick={() => setChallengeCountdown(0)} style={{ flex: 1, padding: '10px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'rgba(255,255,255,0.5)', fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>スルー</button>
          </div>
        </div>
      )}

      {/* ターン表示 */}
      <div style={{ background: isMyTurn ? '#0a2a0a' : '#2a0a0a', borderRadius: 14, padding: '14px 18px', textAlign: 'center' as const, border: `1px solid ${isMyTurn ? 'rgba(74,222,128,0.3)' : 'rgba(229,57,53,0.3)'}` }}>
        <p style={{ fontWeight: 800, fontSize: 17, color: '#fff' }}>
          {isMyTurn ? '🟢 あなたのターン' : `🔴 ${opponent?.name ?? '相手'}のターン`}
        </p>
        {isMyTurn && turnPhase === 'draw_deck' && <p style={{ fontSize: 12, color: '#4ade80', marginTop: 4 }}>山札を自動で引いています...</p>}
        {isMyTurn && turnPhase === 'draw_opponent' && <p style={{ fontSize: 12, color: '#ffd700', marginTop: 4 }}>② 相手の手札からカードを引いてください</p>}
        ...
        <div style={{ background: '#111126', border: '1px solid rgba(155,89,182,0.4)', borderRadius: 14, padding: 14 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 8 }}>
            {availableAbilities.map((ability, i) => {
              ...
              return (
                <button key={i} onClick={() => useAbility(ability)} style={{
      ...
      {selectingHandover && (
        <div style={{ background: '#0a2a1a', border: '1px solid #4ade80', borderRadius: 12, padding: 12 }}>
          <p style={{ fontSize: 13, color: '#4ade80', fontWeight: 700 }}>🤝 相手に渡すカードをタップしてください</p>
        </div>
      )}
      ...
      <div style={{ background: '#111126', borderRadius: 14, padding: 16, border: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          <p style={{ fontWeight: 700, color: '#fff' }}>{opponent?.name ?? '相手'} の手札</p>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 8 }}>
          {opponent?.hand?.map((card, i) => {
            ...
            return (
              <CardDisplay key={i} card={reveal ? card : undefined} faceDown={!reveal}
      ...
      <div style={{ background: '#111126', borderRadius: 14, padding: '12px 16px', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 52, height: 72, borderRadius: 10, background: 'linear-gradient(135deg,#1a1a4e,#0f3460)', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>🎴</div>
        <div>
          <p style={{ fontWeight: 700, fontSize: 14, color: '#fff' }}>山札</p>
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginTop: 2 }}>{game.deck?.length ?? 0}枚残り</p>
          {isMyTurn && turnPhase === 'draw_deck' && <p style={{ fontSize: 11, color: '#4ade80', marginTop: 2 }}>自動で引いています...</p>}
        </div>
      </div>

      {/* 自分の手札 */}
      <div style={{ background: '#111126', borderRadius: 14, padding: 16, border: '1px solid rgba(255,255,255,0.06)' }}>
        <p style={{ marginBottom: 10, fontWeight: 700, fontSize: 14, color: '#fff' }}>
          あなたの手札（{myPlayer?.hand?.length ?? 0}枚）
          {selectingDecoy && <span style={{ fontSize: 12, color: '#FFC107', marginLeft: 8 }}>囮を選択中</span>}
          {selectingHandover && <span style={{ fontSize: 12, color: '#4ade80', marginLeft: 8 }}>渡すカードを選択中</span>}
        </p>
        <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 8 }}>
          {myPlayer?.hand?.map((card, i) => (
            <CardDisplay key={i} card={card}
              highlighted={selectingDecoy || selectingHandover}
              onClick={() => {
                if (selectingDecoy) selectDecoyCard(i);
                else if (selectingHandover) selectHandoverCard(i);
        ...
        <div style={{ background: '#1a0a2e', border: '1px solid #9b59b6', borderRadius: 14, padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <p style={{ fontWeight: 700, fontSize: 15, color: '#fff' }}>{ABILITY_INFO[selectedCard]?.name}</p>
            ...
            <button onClick={() => setSelectedCard(null)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: 20 }}>×</button>
          </div>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', lineHeight: 1.6 }}>{ABILITY_INFO[selectedCard]?.desc}</p>
          {ABILITY_INFO[selectedCard]?.condition && <p style={{ fontSize: 12, color: '#4A90E2', marginTop: 8 }}>🔒 {ABILITY_INFO[selectedCard]?.condition}</p>}
        </div>
      )}

      {/* 能力一覧 */}
      <button onClick={() => setShowGuide(!showGuide)} style={{ padding: '10px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.6)', fontSize: 13, cursor: 'pointer' }}>
        📖 能力一覧 {showGuide ? '▲' : '▼'}
      </button>

      {showGuide && (
        <div style={{ background: '#111126', borderRadius: 14, padding: 16, display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
